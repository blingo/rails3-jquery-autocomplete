# Зачем оно нужно?
В этой версии добавлена возможность поиска сразу по нескольким полям.
Для этого к методу autocomplete в параметрах options нужно передать :columns_names=[:field1, :fileld2]

## Пример применения
Допустим, есть список учреждений. И у каждого учреждения есть сокращенное и полное название, за которые отвечают поля :short_name и :name
Версия гема от авторов позволяла искать данные только по одной колонке с данными, т.е можно было сделать только так:

    autocomplete :archive, :name

Теперь же можно сделать так:

    autocomplete :archive, :name, :columns_names=>[:name, :short_name]

Таким образом, данные будут искаться по столбцам :name и :short_name, а выводиться будут из :name
